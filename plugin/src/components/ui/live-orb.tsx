...
export type LiveOrbOptions = {
  ...
  /** Agent model is actively speaking */
  isSpeaking?: boolean
  /** User is talking / microphone active */
  isListening?: boolean
  /** Continuous microphone volume level (0 to 1) */
  volume?: number
}
...
export function createLiveOrb(
  ...
  const tick = (now: number) => {
    ...
    if (options.isSpeaking) {
      targetSpeed = 1.4 + Math.min(1, options.volume || 0) * 0.8
    } else if (options.isListening) {
      targetSpeed = 0.65 + Math.min(1, options.volume || 0) * 0.6
    } else if (options.status === "connecting") {
      targetSpeed = 0.85 + Math.sin(now * 0.005) * 0.35
    }
    currentSpeed += (targetSpeed - currentSpeed) * 0.12

    gl.uniform1f(uSpeed, reduce ? 0 : currentSpeed)
...
export function LiveOrb({
  ...
  }, [variant, color, eyeColor, colors, interactive, blink, status, isSpeaking, isListening, volume])
  ...
  return (
    ...
    >
      {/* Speaking reactive glow */}
      {isSpeaking && (
        <div className="absolute inset-[-12px] rounded-full animate-pulse opacity-40 bg-gradient-to-tr from-purple-500 via-pink-400 to-indigo-500 blur-xl pointer-events-none -z-10" />
      )}
      {/* Listening reactive pulse ring */}
      {isListening && (
        <div
          className="absolute inset-[-6px] rounded-full transition-all duration-75 border-2 border-emerald-400/60 pointer-events-none -z-10"
          style={{ transform: `scale(${1 + Math.min(0.2, volume * 0.25)})` }}
        />
      )}
